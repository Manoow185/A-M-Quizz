...
function enterLobby(){
  ...
  unsubLobby = onSnapshot(doc(db, 'lobbies', lobbyCode), (snap) => {
    ...
    if(data.status === 'question') renderQuestionScreen(data);
      ...
      document.getElementById('pb-cats').innerHTML = '';
...
function renderCorrectionScreen(data){
  ...
  unsubAnswers = onSnapshot(query(collection(db, 'lobbies', lobbyCode, 'answers'), where('questionIndex', '==', idx)), (qs) => {
    ...
    qs.forEach(d => {
      ...
      if(a.isPetitBac){
        let pbHtml = `<div class="pb-correction-wrap"><div class="answer-head"><span class="avatar">${escapeHtml((a.name||'?').charAt(0).toUpperCase())}</span><span class="who">${escapeHtml(a.name)}</span></div>`;
        Object.entries(a.pbData).forEach(([cat, catData]) => {
          ...
          pbHtml += `<div class="pb-item"><div><strong>${escapeHtml(cat)}</strong><div class="text-bubble small">${escapeHtml(catData.text)}</div></div>
            ${isHost ? `<div class="judge-btns"><button class="yes pb-judge ${btnClassYes}" data-id="${a.id}" data-cat="${cat}" data-val="true">✓</button><button class="no pb-judge ${btnClassNo}" data-id="${a.id}" data-cat="${cat}" data-val="false">✗</button></div>` 
